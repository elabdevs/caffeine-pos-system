    ...
    public static function productBreakdown(): void
{
    try {
        ...
        $onlyPaid = (int)($_GET['onlyPaid'] ?? 1) === 1;      // kapanan siparişler
        ...
        // Kategori ismini almak istersen categories tablosu varsa LEFT JOIN ekleyebilirsin.
        $sql = "
          SELECT 
            p.id           AS product_id,
        ...
        $rows = (new \App\Core\DB('order_items'))->query($sql, $bind, 'array');
        ...
        // categories tablosu yoksa kategori adını boş geçiyoruz
        $items = [];
        foreach ($rows as $r) {
            $items[] = [
                'product_id'   => (int)$r['product_id'],
                'product_name' => $r['product_name'],
                'category'     => null,                 // varsa doldur
                'units_sold'   => (int)$r['units_sold'],
                'net_sales'    => round((float)$r['net_sales'], 2),
                'pct'          => $totalSales > 0 ? round(((float)$r['net_sales'] / $totalSales) * 100, 2) : 0.0,
                'refunds'      => 0,                    // şimdilik 0; ayrı bir refunds tablon varsa bağlarız
            ];
        }
