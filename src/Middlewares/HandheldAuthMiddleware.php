...
final class HandheldAuthMiddleware
{
    ...
    public static function handle(array $opt = []): void
    {
        ...
        $currentVersion = self::dbGetUserSessionVersion(
            (int) $_SESSION["waiter_user_id"]
        );
        if (
            $currentVersion === null ||
            $currentVersion !== ($_SESSION["waiter_session_version"] ?? null)
        ) {
            self::destroyAndFail(
                $apiMode,
                $redirectTo,
    ...
    private static function destroyAndFail(
        bool $api,
        string $redirect,
        int $code,
        string $error
